locals {
  rds_proxy_name                     = "auth-token-generator-dev-proxy"
  db_instance_identifier             = "auth-token-generator-dev-db-db-development"
  rds_proxy_secret_name              = "/api-auth-token-generator/development/rds-proxy-db-credentials"
...
resource "aws_security_group" "rds_proxy" {
  name_prefix = "auth-token-generator-dev-proxy-"
  ...
  tags = {
    Name        = "auth-token-generator-dev-proxy-development"
    Project     = "platform apis"
    Environment = "development"
  }
}
...
resource "aws_db_proxy" "auth_token_generator_development" {
  name                   = local.rds_proxy_name
  engine_family          = "POSTGRESQL"
  role_arn               = aws_iam_role.rds_proxy.arn
  vpc_subnet_ids         = data.aws_subnet_ids.development.ids
...
resource "aws_db_proxy_default_target_group" "auth_token_generator_development" {
  db_proxy_name = aws_db_proxy.auth_token_generator_development.name
...
resource "aws_db_proxy_target" "auth_token_generator_development" {
  db_proxy_name          = aws_db_proxy.auth_token_generator_development.name
  target_group_name      = aws_db_proxy_default_target_group.auth_token_generator_development.name
